...
class TestCoinGeckoProvider(unittest.TestCase):
    ...
    @patch('coingecko_provider.time.time')
    def test_handle_rate_limiting(self, mock_time):
        """Test rate limiting functionality"""
        # Mock time progression
        mock_time.side_effect = [0, 0.5, 1.0]  # Initial, during check, after sleep
        
        self.provider.last_request_time = 0
        self.provider.rate_limit_delay = 1.0
        
        with patch('coingecko_provider.time.sleep') as mock_sleep:
    ...
    def test_format_market_data_success(self):
        ...
        self.assertIn('MarketCap', result.columns)
    ...
    @patch('coingecko_provider.time.sleep')
    ...
    def test_log_api_usage(self):
        ...
        with patch('coingecko_provider.logger') as mock_logger:
...
class TestCoinGeckoProviderIntegration(unittest.TestCase):
    ...
    def test_real_api_ping(self):
        """Test real API ping (requires internet connection)"""
        result = self.provider.get_api_status()
